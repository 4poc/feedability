...
function parse(feed_url, callbacks)
{
  ...
  urlopen.fetch(feed_url, {
    data: function(data, real_url) {
      if(feed_url != real_url) { // the feed url is a redirection
        log.debug('feed url redirected: '+real_url, real_url);
      }
      ...
      xml_parser.addListener('startElement', function(name, attrs) {
        if(!root) {
          if(name == 'feed' || name == 'rss' || name == 'rdf:RDF') {
            root = name;
            ...
            log.info('feed parser confirmed '+type+'-feed', real_url);
          }
      ...
      xml_parser.addListener('endElement', function(name) {
        if(textcue != '') {
          ...
          xml += textcue;
          if(itemurlelm && name == 'link' && !itemurl && 
             !func.string_empty(textcue)) {
        ...
        if(itemelm && func.array_includes(item_elements, name)) {
          log.debug('found item link: '+itemurl, real_url);
