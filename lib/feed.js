...
/**
 * Fetch a feed by url, parseit and create new xml string.
 * 
    ...
    data: function(data) {
      ...
      xml_parser.addListener('startElement', function(name, attrs) {
        if(!root) {
          if(name == 'feed' || name == 'rss') {
            ...
            console.log('--[ '+type+'-feed root element: '+name);
          }
      ...
      xml_parser.addListener('endElement', function(name) {
        ...
        if(itemelm && utils2.inarray(item_elements, name)) {
          console.log('--[ feed parser parsed item link: '+itemurl);
        ...
        if(root == name) {
          callbacks.finished(xml, articles);
        }
      });
      xml_parser.addListener('text', function(text) {        
        // console.log('debug: '+text);
        // if(itemurlelm) itemurl = text; // important: use the decoded url
