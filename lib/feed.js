 ...
 */
var Feed = function(content, url, encoding) {
  // private object properties
  this.content = content;
  // this.encoding = (encoding || this.encodingDecl()).toUpperCase();
  ...
  //
  // expat uses the xml declaration with higher priority then the
  // one specified in the parser, this replaces the encoding in
  // the xml declaration with utf-8
  //
  this.replaceXmlDecl('utf-8');
  require('fs').writeFileSync('encoding_horror', this.content);
  
  log.info('init feed parser for '+url+' ('+this.encoding+')', url);
...
Feed.prototype.replaceXmlDecl = function(encoding) {

  this.content = this.content.replace(/<\?xml ([^>]+) encoding="([^"]+)"([^>]+)>/gi,
    '<?xml $1 encoding="'+encoding+'"$3>'
  );
  log.debug(this.content);
}

/**
 * Parse charset from the xml declaration
 * 
 * @return The encoding specified by declaration element or the fallback(utf8)
 */
Feed.prototype.encodingDecl = function() {
  var fallback = 'UTF-8';
  if(!this.content || this.content.length < 0) return fallback;
  
  var idecl = this.content.indexOf('<?xml ');
  var ldecl = this.content.substr(idecl).indexOf('?>') - idecl;
  if(idecl < 0 || ldecl < 0) return fallback;
  var encoding = (this.content.substr(idecl, ldecl)).match(/encoding="([^"]+)"/i);
  if(encoding) {
    log.debug('charset detected: '+encoding[1]);
    return encoding[1];
  }
  else {
    return fallback;
...
Feed.prototype.parse = function(callback) {
  ...
  require('fs').writeFileSync('foobar42', this.content);
...
Feed.prototype.expatStartElement = function(name, attributes) {
  ...
  if(!this.in_excerpt) {
    ...
    if(!func.object_empty(attributes)) {
      this.stream += ' ' + func.object_proplist(attributes).join(' ');
    }
    this.prev_open = true;
  }
}
 ...
 */
Feed.prototype.createItem = function(item) {
  ...
  if(item.content) {
    content += this.createItemContent(item);
  }
...
Feed.prototype.createItemContent = function(item) {
  var content = '';
  if(this.mime == 'application/atom+xml') {
...
/**
 * Detect the correct mimetype to use.
 * 
 * Sets the mimetype of the document based on the root element.
 * 
 * @param string  The name of the root element
 * @param object  The attributes of the root element
 */
Feed.prototype.detectMime = function(name, attributes) {
  if(name == 'feed') {
    this.mime = 'application/atom+xml';
  }
  else if((name = name.substr(0,3)) == 'rss' || name == 'rdf') {
    this.mime = 'application/rss+xml';
    if(!attributes['xmlns:content']) {
  ...
  else { // just assume some kind of xml
    this.mime = 'application/xml';
  }
  log.debug('feed mime type set: '+this.mime)
}
...
contents = fs.readFileSync('heise-atom.xml', 'utf-8');


var feed = new Feed(contents);
feed.parse(function(err, feed){
  feed.create()
  //console.log();
  
   feed.eachItem(function(item) {
    // this.parallel() // ...
  });
