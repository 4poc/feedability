...
{
  var domain = urllib.parse(url).hostname,
      urlhash = sha1(url);
  var cache_path = settings['cache_path']+'/'+domain;

  if(filestats(cache_path) == null) {
    console.log('create domain directory: '+cache_path);
    fs.mkdirSync(cache_path, 0755);
  }

  return cache_path + '/' + urlhash;
}
...
// returns an array of values that are extracted from an array of hash 
// elements by a specific key
function arrayhashvalue(array, key)
{
  var retarray = Array();
  for(i in array) {
    retarray.push(array[i][key]);
  }
  return retarray;
}
exports.arrayhashvalue = arrayhashvalue;
...
function rel2abs(content, domain) {
