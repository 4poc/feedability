...
Filter.prototype.applyReplaceFilter = function(content, replace_rules) {
  ...
  for(var regex in replace_rules) {
    replace_rules[regex] = replace_rules[regex].replace('%{URL_BASE}', func.url_base(this.url));
    log.debug('replace('+content.length+') filter: regex('+regex+') -> '+replace_rules[regex]);
    content = content.replace((new RegExp(regex, 'ig')), replace_rules[regex]);
