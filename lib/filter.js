...
var jquery_path = utils2.settings['jquery_path'];
var jquery_filters = utils2.settings['filter']['jquery_filters'];
 ...
 */
function filter(contents, callback) {
  var article_urls = utils2.hashkeys(contents);
  ...
  for(var i = 0; i < article_urls.length; i++) {(function() {
      ...
      var cache_file = './cache/'+utils2.sha1(url)+'.rdby';
      if(utils2.filestats(cache_file) !== null) { // cache file exists
      ...
      console.log('filter create jsdom for url: '+url);
      var window = jsdom.jsdom(contents[url]).createWindow();
      ...
      jsdom.jQueryify(window, jquery_path, function(window, jquery) {
        ...
        contents[url] = '<html>'+jquery('html').html()+'</html>';
        next();
      });
